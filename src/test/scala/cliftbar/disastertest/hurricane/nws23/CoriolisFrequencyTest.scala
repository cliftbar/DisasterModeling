...
class CoriolisFrequencyTest extends FunSuite {
  test("Lat 15: coriolis frequency within rounding of 0.0000376") {
  ...
  test("Lat -15: coriolis frequency within rounding of -0.0000376") {
    val result = model.coriolisFrequency(-15) //* 10000 // removing scientific notation
    // Round to 3 decimal places
    assert(result < -0.00003754) // result is within acceptable error of -0.0000376
    assert(-0.00003765 < result) // result is within acceptable error of -0.0000376
  }
}
